...
BOOST_AUTO_TEST_CASE(Power_Functions)
{
    ...
    BOOST_CHECK(Pow<-3>(2.f) == 0.125f);

    BOOST_CHECK(Pow(3, 3) == 27);
    BOOST_CHECK(Pow(3.f, 3) == 27.f);
    BOOST_CHECK(Pow(2.f, -3) == 0.125f);
    GDL_CHECK_THROW_DEV(Pow(2, -3), Exception);
...
BOOST_AUTO_TEST_CASE(IsPowerOf2_Function)
{
    ...
    constexpr I32 numChecks = sizeof(size_t) * 8;

    for (I32 i = 0; i < numChecks; ++i)
    {
