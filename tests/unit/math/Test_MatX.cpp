...
void MultiplicationTest()
{
    ...
    matrix<T, 4, 4> A1(std::array<T, 16>{{0., 4., 8., 12., 1., 5., 9., 13., 2., 6., 10., 14., 3., 7., 11., 15.}});
    matrix<T, 4, 4> B1(std::array<T, 16>{{8., 11., 6., 2., 5., 4., 9., 3., 3., 7., 5., 2., 6., 8., 2., 6.}});

    matrix<T, 4, 4> expC1(std::array<T, 16>{
            {29., 137., 245., 353., 31., 115., 199., 283., 23., 91., 159., 227., 30., 118., 206., 294.}});
    matrix<T, 4, 4> C1 = A1 * B1;
    BOOST_CHECK(checkCloseMat(C1, expC1));
    BOOST_CHECK(checkCloseArray(C1.Data(), expC1.Data()));

    matrix<T, 4, 4> expD1(std::array<T, 16>{
            {116., 168., 100., 100., 138., 198., 122., 113., 160., 228., 144., 126., 182., 258., 166., 139.}});
    matrix<T, 4, 4> D1 = B1 * A1;
    BOOST_CHECK(checkCloseMat(D1, expD1));
    BOOST_CHECK(checkCloseArray(D1.Data(), expD1.Data()));

    // Non square matrix multiplication
    matrix<T, 3, 5> A2(std::array<T, 15>{{0., 5., 10., 1., 6., 11., 2., 7., 12., 3., 8., 13., 4., 9., 14.}});
    matrix<T, 5, 3> B2(std::array<T, 15>{{8., 11., 6., 2., 5., 4., 9., 3., 3., 7., 5., 2., 6., 8., 2.}});

    matrix<T, 3, 3> expC2(std::array<T, 9>{{49., 209., 369., 52., 182., 312., 46., 161., 276.}});
    matrix<T, 3, 3> C2 = A2 * B2;
    BOOST_CHECK(checkCloseMat(C2, expC2));
    BOOST_CHECK(checkCloseArray(C2.Data(), expC2.Data()));

    matrix<T, 5, 5> expD2(std::array<T, 25>{{70.,  65., 75.,  95.,  55.,  87.,  87., 90.,  108., 69.,  104., 109., 105.,
                                             121., 83., 121., 131., 120., 134., 97., 138., 153., 135., 147., 111.}});
    matrix<T, 5, 5> D2 = B2 * A2;
...
void AdditionAssignmentTest()
{
    ...
    matrix<T, 4, 4> expA1(
            std::array<T, 16>{{8., 15., 14., 14., 6., 9., 18., 16., 5., 13., 15., 16., 9., 15., 13., 21.}});
    A1 += B1;
    ...
    matrix<T, 4, 4> expB1(
            std::array<T, 16>{{16., 22., 12., 4., 10., 8., 18., 6., 6., 14., 10., 4., 12., 16., 4., 12.}});
    B1 += B1;
    ...
    matrix<T, 3, 5> A2(std::array<T, 15>{{0., 5., 10., 1., 6., 11., 2., 7., 12., 3., 8., 13., 4., 9., 14.}});
    matrix<T, 3, 5> B2(std::array<T, 15>{{8., 11., 6., 2., 5., 4., 9., 3., 3., 7., 5., 2., 6., 8., 2.}});

    matrix<T, 3, 5> expA2(std::array<T, 15>{{8., 16., 16., 3., 11., 15., 11., 10., 15., 10., 13., 15., 10., 17., 16.}});
    A2 += B2;
    BOOST_CHECK(checkCloseMat(A2, expA2));
    BOOST_CHECK(checkCloseArray(A2.Data(), expA2.Data()));

    matrix<T, 3, 5> expB2(std::array<T, 15>{{16., 22., 12., 4., 10., 8., 18., 6., 6., 14., 10., 4., 12., 16., 4.}});
    B2 += B2;
